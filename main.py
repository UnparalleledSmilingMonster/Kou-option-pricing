...
from psi_fct_kou import european_barrier_uic_option
...
if __name__ == '__main__':    
    ...
    parser.add_argument("--option", type=str, help="The type of option to price. Choose between [european, barrier-uic].", required = True)
    ...
    parser.add_argument("--r", type=float, help="See Kou paper", default=0.05, required=False)
    parser.add_argument("--sigma", type=float, help="See Kou paper", default=0.16, required=False)
    ...
    if args.option == "european":         
        print("Price of the european call option:", european_call_option(args.S0, args.r, args.sigma, args.llambda, args.p, args.eta_1, args.eta_2, args.K, args.T, args.m))       
        
    elif args.option == "barrier-uic":
        print("Price of the UIC barrier option:", european_barrier_uic_option(mp.mpf(args.S0), mp.mpf(args.r), mp.mpf(args.sigma), mp.mpf(args.llambda), mp.mpf(args.p), mp.mpf(args.eta_1), mp.mpf(args.eta_2), mp.mpf(args.K), mp.mpf(args.barrier), mp.mpf(args.T), args.m, args.w)) 
